<nav class="">
    <div class="mx-auto p-8 max-w-screen-xl flex flex-wrap gap-8 items-center justify-between text-white bg-red-600">
        ...
        <a href="/" class="order-2 md:order-3 flex flex-col gap-2 self-center w-full sm:w-auto">
        ...
        <div class="order-3 hidden w-full md:block md:w-auto" id="navbar-default">
            <ul class="bg-red-100 md:bg-red-600 font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg md:rounded-none md:flex-row md:mt-0 md:border-0">
                @foreach ($links as $link)
                    <li>
                        <a href="{{route('frontend.page', [$link->getRelated('page')->first()->slug])}}"
                        class="block py-2 px-3 text-red-600 md:text-white rounded md:rounded-none hover:text-red-700 hover:bg-gray-100 md:hover:bg-transparent
